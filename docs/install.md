...
从 [Releases](https://github.com/nerdneilsfield/flowerss-bot/releases) 页面下载对应的版本解压运行即可。
...
wget -O ~/flowerss/config.yml \
    https://raw.githubusercontent.com/nerdneilsfield/flowerss-bot//master/config.yml.sample
```
...
```shell script
docker run -d -v ~/flowerss:/root/.flowerss nerdneilsfield/flowerss-bot/
```

## 源码编译部署

```shell script
git clone https://github.com/nerdneilsfield/flowerss-bot/ && cd flowerss-bot
make build
./flowerss-bot
```
